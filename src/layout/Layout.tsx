...
import { apiNotifyUnreadList, apiPublicUserInfo } from "@/api/generated";
...
export default async function Layout({
  ...
  let unreadCount = 0;
  if (isLoggedIn) {
    try { avatar = (await apiPublicUserInfo()).avatar || ""; } catch { /* ignore */ }
    try { unreadCount = ((await apiNotifyUnreadList()) || []).length; } catch { /* ignore */ }
  }
  ...
  return (
    <div className="sm:min-h-screen sm:px-6 sm:py-5 lg:py-10">
      <div className="mx-auto flex min-h-screen w-full max-w-5xl flex-col sm:overflow-hidden sm:min-h-[calc(100vh-40px)] sm:rounded-2xl sm:border sm:border-(--border) sm:bg-(--surface) sm:shadow-(--shadow) lg:min-h-[calc(100vh-80px)]">
        <NavBar isLoggedIn={isLoggedIn} avatar={avatar} unreadCount={unreadCount} />
